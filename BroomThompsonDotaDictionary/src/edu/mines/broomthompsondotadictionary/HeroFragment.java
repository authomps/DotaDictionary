...
public class HeroFragment extends Fragment {
	...
	public void updateHeroView(String name) {
		...
		if (hero.getName().length() > 13 && getActivity().findViewById(R.id.fragment_container) != null) {
		...
		// Display abilities
		LinearLayout abilities_ll = (LinearLayout) getActivity().findViewById(R.id.abilities);
		...
		for (int i=0; i < abilities.length-2; i=i+3) {
	        ...
	        Future<Bitmap> ability_result = ability_es.submit(new Callable<Bitmap>() {
				public Bitmap call() throws Exception {
	        ...
	        try {
				...
				// If exception, make log statement and print stack
	        	ability_img = BitmapFactory.decodeResource(getActivity().getResources(),R.drawable.img_portrait_unknown);
			...
			ability_line.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, 1));
			ability_line.setBackgroundColor(getActivity().getResources().getColor(R.color.red));
